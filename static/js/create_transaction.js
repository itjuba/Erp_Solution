    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.form_is_valid) {
          ...
          md.showNotificationSuccess('top','center','success','Transaction Created Successfully !');
          console.log("success");
        }
        else {
          console.log('error');
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.form_is_valid) {
          $("#book-table tbody").html(data.html_book_list);
          $("#modal-book").modal("hide");
          md.showNotificationDelete('top','center','danger','Transaction Deleted Successfully !');
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.form_is_valid) {
          $("#book-table tbody").html(data.html_book_list);
          $("#modal-book").modal("hide");
          md.showNotificationUpdate('top','center','primary','Transaction Updated Successfully');
          setTimeout(function() { $("#display").hide(); }, 5000);
        }
        else {
          $("#modal-book .modal-content").html(data.html_form);
